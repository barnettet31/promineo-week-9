...
import { everyNthElement } from "./utilities";
...
myMortgageForm.addEventListener("submit", (e) => {
  ...
  const MONTHLY_PAYMENT =
    (PRINCIPAL *
      (MONTHLY_INTEREST_RATE *
        (1 + MONTHLY_INTEREST_RATE) ** TOTAL_NUMBER_OF_MONTHS)) /
    ((1 + MONTHLY_INTEREST_RATE) ** TOTAL_NUMBER_OF_MONTHS - 1);
  const mortgageData = Array.from(
    { length: TOTAL_NUMBER_OF_MONTHS },
    (_, i) => {
  ...
  const dataToSendToChart = [
    ...everyNthElement(mortgageData, 12).filter(
      (data) => data.year != mortgageData[mortgageData.length - 1].year
    ),
    mortgageData[mortgageData.length - 1],
  ];
